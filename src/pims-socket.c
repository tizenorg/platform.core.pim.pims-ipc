...
static int __recv_raw_data(int fd, pims_ipc_raw_data_s **data, int *init)
{
	...
	do {
		// total length
		ret = read(fd, &total_len, sizeof(total_len));
		if (-1 == ret) {
		...
		// client_id
		ret = read(fd, &(temp->client_id_len), sizeof(temp->client_id_len));
		if (-1 == ret) {
			ERR("read() Fail(%d)", errno);
			break;
		}
		...
		// sequnce no
		ret = read(fd, &(temp->seq_no), sizeof(temp->seq_no));
		if (-1 == ret) {
			ERR("read() Fail(%d)", errno);
			break;
		}
		...
		// call_id
		ret  = read(fd, &(temp->call_id_len), sizeof(temp->call_id_len));
		if (-1 == ret) {
			ERR("read() Fail(%d)", errno);
			break;
		}
		...
		// has_data
		ret = read(fd, &has_data, sizeof(has_data));
		if (-1 == ret) {
			ERR("read() Fail(%d)", errno);
			break;
		}
		...
		if (has_data) {
			temp->has_data = TRUE;
			ret = read(fd, &(temp->data_len), sizeof(temp->data_len));
			if (-1 == ret) {
				ERR("read() Fail(%d)", errno);
				break;
			}
