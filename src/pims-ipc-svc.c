...
static pims_ipc_client_info_s* _clone_client_info(pims_ipc_client_info_s *client_info)
{
	...
	clone->smack = g_strdup(client_info->smack);
	if (NULL == clone->smack)
		ERROR("g_strdup() Fail");

	clone->uid = g_strdup(client_info->uid);
	if (NULL == clone->uid)
		ERROR("g_strdup() Fail");

	clone->client_session = g_strdup(client_info->client_session);
...
API bool pims_ipc_svc_check_privilege(pims_ipc_h ipc, char *privilege)
{
	...
	pthread_mutex_lock(&_g_singleton->cynara_mutex);
 	ret = cynara_check(_g_singleton->cynara, client_info->smack, client_info->client_session, client_info->uid, privilege);
	pthread_mutex_unlock(&_g_singleton->cynara_mutex);
...
API int pims_ipc_svc_get_smack_label(pims_ipc_h ipc, char **p_smack)
{
	...
	*p_smack = g_strdup(client_info->smack);
	if (NULL == p_smack) {
